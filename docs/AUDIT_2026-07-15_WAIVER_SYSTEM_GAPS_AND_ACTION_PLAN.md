...
## Phase 5 status (updated 2026-07-15)

| Task | Status |
|---|---|
| SMS waiver automations (G10) | **Done.** Messageistic gains two lifecycle triggers: `waiver.signed` (hooks Phase 3's `memberistic_waiver_signed` — confirmation text with validity date, silently skipped when the signer left no phone) and `waiver.renewal_due` (hooks a new `memberistic_waiver_renewal_due` action fired by the daily renewal pass alongside the email, sharing its once-per-signing-cycle guarantee; carries the member's tokenized sign link). The pre-existing `waiver.incomplete` trigger (self check-in) was already wired. |
| Workflow pack presets | **Done.** Two new Firearm Workflow Pack templates — "Waiver signed confirmation" and "Waiver renewal due" — installed inactive (consent + opt-out conditions attached) for staff to switch on, consistent with the existing pack entries. |
| Weekly ops digest | **Done.** The weekly business report now includes a waiver-operations line: signings this week (kiosk share), waivers expiring in 30 days, and upcoming bookings with no waiver on file. Degrades to nothing when Memberistic's tables are absent. |

**With Phase 5, all phases (0–5) of this action plan are implemented.** Remaining open item: ops task 0.1 (repo/live plugin reconcile before the next deploy).
