...
anyone who booked a range lane. That hook has been removed (see
...
- either the membership was created within ±10 minutes of a
  `wp_g2ab_bookings` row for the same customer email, **or** `created_by`
...
5. Adds user meta `g2ab_customer_segment = range_guest` to the primary user.
6. Leaves the WP user, `wp_g2ab_bookings` rows, payments, waiver records and
...
- deletes the `g2ab_customer_segment` user meta, but only when the original
  run set it and it still equals `range_guest` (a later manual change wins).
