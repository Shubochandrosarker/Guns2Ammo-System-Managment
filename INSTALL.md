...
| G2A POS Core plugin | `g2a-pos-core.zip` (current archive `g2a-pos-core-3.3.6.zip`) | **3.3.6** ✅ (dual-account hardening, integrity checks, PHP 8.1+) |
...
8. **G2A POS Core** (v3.3.6) — full FFL POS with dual-account support and integrity checking (requires PHP 8.1+; composer vendors ship inside the zip). Upload, activate, then enable the **POS Bridge** module in `Memberistic → Integrations` so the counter sees live membership status.
