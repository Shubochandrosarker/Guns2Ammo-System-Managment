...
_Last updated: 2026-07-12 · Plugin version 1.14.0 · DB schema 1.11.0_
...
- **`guns2ammo-system-managment`** (this repo — "guns2ammo-complete-
  custom-business-system" elsewhere in this doc is a long-standing
  nickname, not the literal repo name) — source of truth. New features
  land here first, in principle.
- **`ffl-checkout--solutions`** — mirrored from the above, in principle.

**In practice, v1.10.0 through v1.14.0 were all built in
`ffl-checkout--solutions` only**, and this repo independently picked up
its own real fixes in the meantime (a "system audit" PR #73 here, bumping
plugin patch versions 1.9.0→1.9.4 / schema 1.4.0→1.4.1 with no matching
changelog or STATUS.md entries — don't trust a stale "last known version"
claim in either repo's docs without re-running `diff -rq`, see §7). Before
this update this repo was missing: a server-side Verifyistic
age-verification gate at checkout, the v1.10.0-v1.13.0 feature work
(Exhibits 01-10, gaps #11-15), and more; `ffl-checkout--solutions` was
missing this repo's own age-verification gate, dealer-endpoint rate-limit
+ notes-leak fix, holiday-aware NICS/multi-sale math, a theme Customizer
key bug fix, an iOS zoom CSS fix, and atomic race guards on transfer
creation and two status-advance paths.

**This update reconciles both directions**: this repo's independent fixes
were backported into `ffl-checkout--solutions` first (adapting the old
single-transfer race-guard to that repo's newer multi-item architecture
where the two conflicted), making it a strict superset of this repo's
real work; that reconciled tree was then copied forward into this repo
wholesale. As of this commit, `diff -rq` between the two repos'
`advanced-ffl-checkout/` trees returns **zero functional differences**
again (aside from `vendor/`, `.phpunit.cache/`, and each repo's own
`.gitignore`/CI-workflow conventions for the test suite). Keep it that way
— mirror every future commit to the other repo in the same session.
...
The v1.10.0-v1.14.0 work above developed on `claude/status-new-features-
upz5ix` in ffl-checkout--solutions across several PRs, then ported forward
into this repo (`guns2ammo-system-managment` — the actual repo name; the
"complete-custom-business-system" name elsewhere in this doc is a
long-standing nickname, not the literal repo) on branch
`claude/port-ffl-checkout-v1.14.0`.
...
1. **Cross-repo reconciliation done — verify it stuck.** This repo and
   `ffl-checkout--solutions` were brought back to zero functional
   differences (see §1). Confirm this PR merges cleanly and re-run
   `diff -rq` against `ffl-checkout--solutions` afterward. Still open: port
   the g2a-booking-engine module + the sibling-repo addon PRs mentioned in
   §1 into their own repos' copies, if mirrored anywhere.
