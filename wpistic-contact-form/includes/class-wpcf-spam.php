...
class WPISTIC_CF_Spam {
	 ...
	 *
	 * The read-check-increment-write is wrapped in a MySQL advisory lock so
	 * a scripted concurrent burst from one IP can't have two requests both
	 * read the same stale count and both slip through — without the lock,
	 * that lost-update race lets a burst exceed `$max` regardless of what
	 * it's configured to.
	 *
	 * @param string $ip Client IP.
	...
	private static function acquire_lock( $key, $timeout = 3 ) {
		...
		return 1 === (int) $wpdb->get_var( $wpdb->prepare( 'SELECT GET_LOCK(%s, %d)', 'wpcf_rl_lock_' . $key, max( 0, $timeout ) ) );
	}
	...
	private static function release_lock( $key ) {
		...
		$wpdb->get_var( $wpdb->prepare( 'SELECT RELEASE_LOCK(%s)', 'wpcf_rl_lock_' . $key ) );
	}
